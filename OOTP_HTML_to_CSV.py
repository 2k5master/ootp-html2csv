...
button3 = Button(root, text="Convert to CSV and Get Underperforming Players (SP)", width=45, command=lambda: option3())
button3.place(x=5, y=165)
# Entry Fields
entry1 = Entry(root, width=37)
entry1.place(x=105, y=45)
result = Entry(root, width=46)
result.place(x=48, y=200)
# Option Menu
...
def function(main_menu_input):
    ...
    try:
        ...
        names = names_list[2::5]
        positions = names_list[0::5]
        ...
        with open('output.csv', 'w', newline='') as csvfile:
            ...
            for i in range(0, length):
                filecsv.writerow(names_list[0:5:] + stats_list[0:25:])
                for i in range(0, 25):
                    stats_list.pop(0)
                for i in range(0, 5):
                    names_list.pop(0)
